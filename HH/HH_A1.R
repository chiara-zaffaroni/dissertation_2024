...
HH_plate_A1_original <- read_excel("hMglia HH A1 CM HiVa4.xlsx", sheet = 1, skip = 8) %>% 
  clean_names() %>% 
...
HH_LPS_map_A1 <- read_excel("hMglia HH A1 CM HiVa4.xlsx", sheet = 3) %>% 
  mutate(row_names = .[[1]]) %>%
  select(-1) %>%
  column_to_rownames(var = "row_names") %>% 
  gather(key = "Column", value = "Value", everything())

HH_platemap_A1 <- read_excel("hMglia HH A1 CM HiVa4.xlsx", sheet = 2, skip = 3) %>% 
  rename(Row = "...1") %>%
  mutate(row_names = .[[1]]) %>%
  select(-1) %>%
  column_to_rownames(var = "row_names") %>% 
  gather(key = "Column", value = "Compound", everything()) %>% 
  bind_cols(HH_LPS_map_A1$Value) %>% 
  rename(LPS_status = "...3")



HH_plate_A1 <- HH_plate_A1_original %>% 
  bind_cols(HH_platemap_A1[c("Compound", "LPS_status")]) %>% 
  # rename(Compound = "...49") %>%
  select(Media_Type, Row, Column, Compound, LPS_status, Nuclei_Count, everything())

# export into an excel doc
write_xlsx(HH_plate_A1, "final_hMglia HH A1 CM HiVa4.xlsx")


HH_plate_A1 <- read_excel("final_hMglia HH A1 CM HiVa4.xlsx") %>% 
  clean_names()

HH_plate_A1_max <- HH_plate_A1 %>% 
  filter(compound == "dmso", lps_status == "LPS") %>% 
  pivot_longer(cols = contains("area"), names_to = "Variable", values_to = "Value") %>% 
  select(Variable, Value)


HH_A1_CV_analysis <- HH_plate_A1_max %>% 
  group_by(Variable) %>% 
  # select(Variable, Value) %>% 
  summarise(CV = sd(Value) / mean(Value) * 100) %>% 
  gt() %>% 
  tab_header(title = "Plate HH A1") %>%
