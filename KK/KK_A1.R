...
KK_plate_A1_original <- read_excel("hMglia KK A1 24hrs Bari Tacro.xlsx", sheet = 1) %>% 
  clean_names() %>%
  ...
  select(-c(cell_10k_objects_rod_h_mglia_dapi_cy5_13, cell_10k_objects_rod_h_mglia_dapi_cy5_72,cell_12_5k_objects_rod_h_mglia_dapi_cy5_106,
            ...
            measurement_set_id, plate_id, cell_count_rod_h_mglia_dapi_cy5_4, cell_object_id_rod_h_mglia_dapi_cy5_5, cell_object_id_rod_h_mglia_dapi_cy5_64, cell_count_rod_h_mglia_dapi_cy5_63)) %>%
  select(1:47) %>% 
  mutate_at(vars(4:47), ~ . / Nuclei_Count)


KK_LPS_map_A1 <- read_excel("hMglia KK A1 24hrs Bari Tacro.xlsx", sheet = 4) %>% 
  mutate(row_names = .[[1]]) %>%
  select(-1) %>%
  column_to_rownames(var = "row_names") %>% 
  gather(key = "Column", value = "Value", everything())

KK_platemap_A1 <- read_excel("hMglia KK A1 24hrs Bari Tacro.xlsx", sheet = 3) %>% 
  rename(Row = "...1") %>%
  mutate(row_names = .[[1]]) %>%
  select(-1) %>%
  column_to_rownames(var = "row_names") %>% 
  gather(key = "Column", value = "Compound", everything()) %>% 
  bind_cols(KK_LPS_map_A1$Value) %>% 
  rename(LPS_status = "...3")

KK_plate_A1 <- KK_plate_A1_original %>% 
  bind_cols(KK_platemap_A1[c("Compound", "LPS_status")]) %>% 
  # rename(Compound = "...49") %>%
  select(row, column, Compound, LPS_status, Nuclei_Count, everything())

# export into an excel doc
write_xlsx(KK_plate_A1, "final_hMglia KK A1.xlsx")     

KK_plate_A1 <- read_excel("final_hMglia KK A1.xlsx")

# Area analysis
KK_plate_A1_area <- KK_plate_A1 %>% 
  select(contains("area"))

KK_plate_A1_area_long <- KK_plate_A1 %>% 
  pivot_longer(cols = contains("area"), names_to = "Variable", values_to = "Value") %>% 
  select(Variable, Value)

KK_plate_A1_area_plot <- KK_plate_A1_area_long %>% 
  ...
  labs(title = "Plate KK A1 (MGM 24h)",
       x = "Threshold",
       y = "CD38 Area/nuclei") +
  theme_minimal() +
...
KK_A1_Z_plot <- KK_A1_Z_data %>% 
  ...
  labs(title ="Plate KK A1 MGM 24h (with edges)", y = "CD38 Area/nuclei") 

# Group data by Threshold
grouped_data_KK_A1 <- KK_A1_Z_data %>%
  group_by(Threshold)
...
z_prime_results_KK_A1 <- grouped_data_KK_A1 %>%
  reframe(
  ...
  gt() %>% 
  tab_header(title = "Plate KK A1 MGM 24h (with edges)")



# edgeless plate
KK_plate_A1_edgeless <- KK_plate_A1 %>% 
  filter(!column %in% c("1", "2", "23", "24")) %>% 
  filter(!row %in% c("A", "B", "P", "O")) 
KK_plate_A1_edgeless_max <- KK_plate_A1_edgeless %>% 
  filter(Compound == "dmso", LPS_status == "LPS") %>% 
  pivot_longer(cols = contains("area"), names_to = "Variable", values_to = "Value") %>% 
  select(Variable, Value)

# CV for max only (edgeless)
KK_edgeless_CV_analysis <- KK_plate_A1_edgeless_max %>% 
  group_by(Variable) %>% 
  # select(Variable, Value) %>% 
  summarise(CV = sd(Value) / mean(Value) * 100) %>% 
  gt() %>% 
  tab_header(title = "KK A1 MGM 24h (without edges)") %>%
  fmt_number(decimals = 2) %>% 
  tab_style(
    style = cell_text(
      size = "smaller",
...
KK_A1_Z_data_edgeless <- KK_plate_A1_edgeless %>% 
  filter(Compound %in% c("dmso", "TAK3mM")) %>% 
  mutate(Treatment = interaction(LPS_status, Compound)) %>% 
...
# Plot (no edges)
KK_A1_Z_plot_edgeless <- KK_A1_Z_data_edgeless %>% 
  ggplot(aes(x=Treatment, y=Value, fill = Treatment)) +
  geom_boxplot() +
  geom_jitter(position = position_jitter(width = 0.2), size = 1, alpha = 0.5) +
  facet_grid(~ Threshold, labeller = labeller(Threshold = c("cell_area_1k_rod_h_mglia_dapi_cy5_23" = "1k Threshold", "cell_area_2k_rod_h_mglia_dapi_cy5_26" = "2k Threshold", "cell_area_3_5k_rod_h_mglia_dapi_cy5_30" = "3.5k Threshold", "cell_area_5k_rod_h_mglia_dapi_cy5_33" = "5k Threshold"))) +
  # theme_minimal() +
  theme(legend.position = "none", strip.text = element_text(size = 12, face = "bold"), axis.title=element_text(size=13)) +
  labs(title ="Plate KK A1 MGM 24h (without edges)", y = "CD38 Area/nuclei") 

# Group data by Threshold
grouped_data_KK_A1_edgeless <- KK_A1_Z_data_edgeless %>%
  group_by(Threshold)
...
z_prime_results_KK_A1_edgeless <- grouped_data_KK_A1_edgeless %>%
  reframe(
    Z_Prime_LPS_TAK3uM = calculate_z_prime(
      Median[Treatment == "LPS.dmso"],
      SD[Treatment == "LPS.dmso"],
  ...
  gt() %>% 
  tab_header(title = "Plate KK A1 MGM 24h (without edges)")


# Baricitinib 1K threshold
KK_A1_Z_data_Bari <- KK_plate_A1_edgeless %>% 
...
# Bari Plot
KK_A1_Z_Bari_plot <- KK_A1_Z_data_Bari %>% 
  ...
  theme(legend.position = "none")+
  labs(title ="Plate KK A1 (MGM) Baricitinib (24h)", y = "CD38 Area/nuclei")

# checking assumptions for 1 factor GLM
# Specify reference level for the category variable
KK_A1_Z_data_Bari$Treatment <- relevel(KK_A1_Z_data_Bari$Treatment, ref = "LPS.dmso")
Bari_lm <- lm(Value ~ factor(Treatment), data = KK_A1_Z_data_Bari)
plot(Bari_lm)

summary(Bari_lm)
anova(Bari_lm)
comparisons_list <- list(c("LPS.dmso", "LPS.Bari 0.3"), c("LPS.dmso", "LPS.Bari 1"), c("LPS.dmso", "LPS.Bari 3"))
KK_A1_Z_Bari_plot + geom_signif(comparisons = comparisons_list, map_signif_level = TRUE, step_increase = 0.1, colour = "black")


# Tacro 1K threshold
KK_A1_Z_data_Tacro <- KK_plate_A1_edgeless %>% 
...
# Tacro Plot
KK_A1_Z_Tacro_plot <- KK_A1_Z_data_Tacro %>% 
  ...
  facet_wrap(~ Threshold) + 
  theme(legend.position = "none")+
  labs(title ="Plate KK A1 (MGM) Tacrolimus (24h)", y = "CD38 Area/nuclei")

Tacro_lm <- lm(Value ~ factor(Treatment), data = KK_A1_Z_data_Tacro)
# plot(Tacro_lm)
summary(Tacro_lm)
# anova(Tacro_lm)
comparisons_list <- list(c("LPS.dmso", "LPS.Tac 0.3"), c("LPS.dmso", "LPS.Tac 1"), c("LPS.dmso", "LPS.Tac 3"))
KK_A1_Z_Tacro_plot + geom_signif(comparisons = comparisons_list, map_signif_level = TRUE, step_increase = 0.1, colour = "black")


# Selinexor 1K threshold
KK_A1_Z_data_Sel <- KK_plate_A1_edgeless %>% 
...
# Selinexor Plot
KK_A1_Z_Sel_plot <- KK_A1_Z_data_Sel %>% 
  ...
  facet_wrap(~ Threshold) + 
  theme(legend.position = "none")+
  labs(title ="Plate KK A1 (MGM) ) Selinexor (24h)", y = "CD38 Area/nuclei")

Sel_lm <- lm(Value ~ factor(Treatment), data = KK_A1_Z_data_Sel)
# plot(Sel_lm)
summary(Sel_lm)
# anova(Sel_lm)
comparisons_list <- list(c("LPS.dmso", "LPS.Sel 0.3"), c("LPS.dmso", "LPS.Sel 1"), c("LPS.dmso", "LPS.Sel 3"))
KK_A1_Z_Sel_plot + geom_signif(comparisons = comparisons_list, map_signif_level = TRUE, step_increase = 0.1, colour = "black")


# Everolimus 1K threshold
KK_A1_Z_data_Evero <- KK_plate_A1_edgeless %>% 
...
# Everolimus Plot
KK_A1_Z_Evero_plot <- KK_A1_Z_data_Evero %>% 
  ...
  facet_wrap(~ Threshold) + 
  theme(legend.position = "none")+
  labs(title ="Plate KK A1 (MGM) Everolimus (24h)", y = "CD38 Area/nuclei")

Evero_lm <- lm(Value ~ factor(Treatment), data = KK_A1_Z_data_Evero)
# plot(Evero_lm)
summary(Evero_lm)
# anova(Evero_lm)
