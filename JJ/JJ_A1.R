...
JJ_plate_A1_original <- read_excel("hMglia JJ A1.xlsx", sheet = 1) %>% 
  clean_names() %>% 
  arrange(column) %>% 
...
JJ_LPS_map_A1 <- read_excel("hMglia JJ A1.xlsx", sheet = 3) %>% 
  mutate(row_names = .[[1]]) %>%
  select(-1) %>%
  column_to_rownames(var = "row_names") %>% 
  gather(key = "Column", value = "Value", everything())

JJ_platemap_A1 <- read_excel("hMglia JJ A1.xlsx", sheet = 2, skip = 8) %>% 
  rename(Row = "...1") %>%
  mutate(row_names = .[[1]]) %>%
  select(-1) %>%
  column_to_rownames(var = "row_names") %>% 
  gather(key = "Column", value = "Compound", everything()) %>% 
  bind_cols(JJ_LPS_map_A1$Value) %>% 
  rename(LPS_status = "...3")

JJ_plate_A1 <- JJ_plate_A1_original %>% 
  bind_cols(JJ_platemap_A1[c("Compound", "LPS_status")]) %>% 
  # rename(Compound = "...49") %>%
  select(media_type, row, column, Compound, LPS_status, Nuclei_Count, everything())

# export into an excel doc
write_xlsx(JJ_plate_A1, "final_hMglia JJ A1.xlsx")     


# Area analysis
JJ_plate_A1_max <- JJ_plate_A1 %>% 
  filter(Compound == "dmso", LPS_status == "LPS") %>% 
  pivot_longer(cols = contains("area"), names_to = "Variable", values_to = "Value") %>% 
  select(Variable, Value)

JJ_plate_A1_area_plot <- JJ_plate_A1_max %>% 
...
JJ_A1_Z_plot <- JJ_A1_Z_data %>% 
  ...
  labs(title ="Plate JJ A1 CM 24h (with edges)", y = "CD38 Area/nuclei") +
...
grouped_data_JJ_A1 <- JJ_A1_Z_data %>%
  group_by(Threshold)
...
z_prime_results_JJ_A1 <- grouped_data_JJ_A1 %>%
  reframe(
  ...
  gt() %>% 
  tab_header(title = "Plate JJ A1 CM 24h (with edges)")



# edgeless plate
JJ_plate_A1_edgeless <- JJ_plate_A1 %>% 
  filter(!column %in% c("1", "2", "23", "24")) %>% 
  filter(!row %in% c("A", "B", "P", "O")) 
JJ_plate_A1_edgeless_max <- JJ_plate_A1_edgeless %>% 
  filter(Compound == "dmso", LPS_status == "LPS") %>% 
  pivot_longer(cols = contains("area"), names_to = "Variable", values_to = "Value") %>% 
  select(Variable, Value)

# CV for max only (edgeless)
JJ_edgeless_CV_analysis <- JJ_plate_A1_edgeless_max %>% 
  group_by(Variable) %>% 
  # select(Variable, Value) %>% 
  summarise(CV = sd(Value) / mean(Value) * 100) %>% 
  gt() %>% 
  tab_header(title = "JJ A1 CM 24h (without edges)") %>%
  fmt_number(decimals = 2) %>% 
  tab_style(
    style = cell_text(
      size = "smaller",
...
JJ_A1_Z_data_edgeless <- JJ_plate_A1_edgeless %>% 
  filter(Compound %in% c("dmso", "TAK3mM")) %>% 
  mutate(Treatment = interaction(LPS_status, Compound)) %>% 
...
# Plot (no edges)
JJ_A1_Z_plot_edgeless <- JJ_A1_Z_data_edgeless %>% 
  ggplot(aes(x=Treatment, y=Value, colour = Treatment)) +
  geom_boxplot() +
  theme_minimal() +
  ...
  theme(legend.position = "none", strip.text = element_text(size = 12, face = "bold"), axis.title=element_text(size=13)) +
  labs(title ="Plate JJ A1 CM 24h (without edges)", y = "CD38 Area/nuclei") 

# Group data by Threshold
grouped_data_JJ_A1_edgeless <- JJ_A1_Z_data_edgeless %>%
  group_by(Threshold)
...
z_prime_results_JJ_A1_edgeless <- grouped_data_JJ_A1_edgeless %>%
  reframe(
    Z_Prime_LPS_TAK3uM = calculate_z_prime(
      Median[Treatment == "LPS.dmso"],
      SD[Treatment == "LPS.dmso"],
  ...
  gt() %>% 
  tab_header(title = "Plate JJ A1 CM 24h (without edges)")


# Baricitinib 1K threshold
JJ_A1_Z_data_Bari <- JJ_plate_A1_edgeless %>% 
...
# Bari Plot
JJ_A1_Z_Bari_plot <- JJ_A1_Z_data_Bari %>% 
  ...
  theme(legend.position = "none")+
  labs(title ="Plate JJ A1 (CM) Baricitinib (24h)", y = "CD38 Area/nuclei")

JJ_A1_Z_data_Bari$Treatment <- relevel(JJ_A1_Z_data_Bari$Treatment, ref = "LPS.dmso")
Bari_lm <- lm(Value ~ factor(Treatment), data = JJ_A1_Z_data_Bari)
plot(Bari_lm)
summary(Bari_lm)
anova(Bari_lm)

comparisons_list <- list(c("LPS.dmso", "LPS.Bari 0.3"), c("LPS.dmso", "LPS.Bari 1"), c("LPS.dmso", "LPS.Bari 3"))
JJ_A1_Z_Bari_plot + geom_signif(comparisons = comparisons_list, map_signif_level = TRUE, step_increase = 0.1, colour = "black")

# Tacro 1K threshold
JJ_A1_Z_data_Tacro <- JJ_plate_A1_edgeless %>% 
...
# Tacro Plot
JJ_A1_Z_Tacro_plot <- JJ_A1_Z_data_Tacro %>% 
  ...
  theme_minimal() +
  theme(legend.position = "none")+
  labs(title ="Plate JJ A1 (CM) Tacrolimus (24h)", y = "CD38 Area/nuclei")

JJ_A1_Z_data_Tacro$Treatment <- relevel(JJ_A1_Z_data_Tacro$Treatment, ref = "LPS.dmso")
Tacro_lm <- lm(Value ~ factor(Treatment), data = JJ_A1_Z_data_Tacro)
plot(Tacro_lm)
summary(Tacro_lm)
anova(Tacro_lm)

comparisons_list <- list(c("LPS.dmso", "LPS.Tac 0.3"), c("LPS.dmso", "LPS.Tac 1"), c("LPS.dmso", "LPS.Tac 3"))
JJ_A1_Z_Tacro_plot + geom_signif(comparisons = comparisons_list, map_signif_level = TRUE, step_increase = 0.1, colour = "black")


# Selinexor 1K threshold
JJ_A1_Z_data_Sel <- JJ_plate_A1_edgeless %>% 
...
# Selinexor Plot
JJ_A1_Z_Sel_plot <- JJ_A1_Z_data_Sel %>% 
  ...
  theme_minimal() +
  theme(legend.position = "none") +
  labs(title ="Plate JJ A1 (CM) Selinexor (24h)", y = "CD38 Area/nuclei")

JJ_A1_Z_data_Sel$Treatment <- relevel(JJ_A1_Z_data_Sel$Treatment, ref = "LPS.dmso")
Sel_lm <- lm(Value ~ factor(Treatment), data = JJ_A1_Z_data_Sel)
plot(Sel_lm)
summary(Sel_lm)
anova(Sel_lm)

comparisons_list <- list(c("LPS.dmso", "LPS.Sel 0.3"), c("LPS.dmso", "LPS.Sel 1"), c("LPS.dmso", "LPS.Sel 3"))
JJ_A1_Z_Sel_plot + geom_signif(comparisons = comparisons_list, map_signif_level = TRUE, step_increase = 0.1, colour = "black")


# Everolimus 1K threshold
JJ_A1_Z_data_Evero <- JJ_plate_A1_edgeless %>% 
...
# Everolimus Plot
JJ_A1_Z_Evero_plot <- JJ_A1_Z_data_Evero %>% 
  ...
  theme_minimal() +
  theme(legend.position = "none")+
  labs(title ="Plate JJ A1 (CM) Everolimus (24h)", y = "CD38 Area/nuclei")

JJ_A1_Z_data_Evero$Treatment <- relevel(JJ_A1_Z_data_Evero$Treatment, ref = "LPS.dmso")
Evero_lm <- lm(Value ~ factor(Treatment), data = JJ_A1_Z_data_Evero)
plot(Evero_lm)
summary(Evero_lm)
anova(Evero_lm)

comparisons_list <- list(c("LPS.dmso", "LPS.Ever 0.3"), c("LPS.dmso", "LPS.Ever 1"), c("LPS.dmso", "LPS.Ever 3"))
JJ_A1_Z_Evero_plot + geom_signif(comparisons = comparisons_list, map_signif_level = TRUE, step_increase = 0.1, colour = "black")
