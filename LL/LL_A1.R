...
LL_LPS_map_A1 <- read_excel("hMglia LL A1 BariTacro Round4 15.3.24.xlsx", sheet = 3) %>% 
  mutate(row_names = .[[1]]) %>%
  select(-1) %>%
  column_to_rownames(var = "row_names") %>% 
  gather(key = "Column", value = "Value", everything())

LL_platemap_A1 <- read_excel("hMglia LL A1 BariTacro Round4 15.3.24.xlsx", sheet = 2) %>% 
  rename(Row = "...1") %>%
  mutate(row_names = .[[1]]) %>%
  select(-1) %>%
  column_to_rownames(var = "row_names") %>% 
  gather(key = "Column", value = "Compound", everything()) %>% 
  bind_cols(LL_LPS_map_A1$Value) %>% 
  rename(LPS_status = "...3")

LL_plate_A1 <- LL_plate_A1_original %>% 
  bind_cols(LL_platemap_A1[c("Compound", "LPS_status")]) %>% 
  # rename(Compound = "...49") %>%
  select(row, column, Compound, LPS_status, Nuclei_Count, everything())

# export into an excel doc
write_xlsx(LL_plate_A1, "final_hMglia LL A1 BariTacro Round4 15.3.24.xlsx")     

LL_plate_A1 <- read_excel("final_hMglia LL A1 BariTacro Round4 15.3.24.xlsx")
...
LL_plate_A1_max <- LL_plate_A1 %>% 
  filter(Compound == "dmso", LPS_status == "LPS") %>% 
  pivot_longer(cols = contains("area"), names_to = "Variable", values_to = "Value") %>% 
  select(Variable, Value)

LL_plate_A1_area_plot <- LL_plate_A1_max %>% 
...
LL_A1_CV_analysis <- LL_plate_A1_max %>% 
  group_by(Variable) %>% 
  # select(Variable, Value) %>% 
  summarise(CV = sd(Value) / mean(Value) * 100) %>% 
  gt() %>% 
  tab_header(title = "Plate LL A1") %>%
...
# edgeless plate
LL_plate_A1_edgeless <- LL_plate_A1 %>% 
  filter(!column %in% c("1", "2", "23", "24")) %>% 
  filter(!row %in% c("A", "B", "P", "O")) 
LL_plate_A1_edgeless_max <- LL_plate_A1_edgeless %>% 
  filter(Compound == "dmso", LPS_status == "LPS") %>% 
  pivot_longer(cols = contains("area"), names_to = "Variable", values_to = "Value") %>% 
  select(Variable, Value)

# CV for max only (edgeless)
LL_edgeless_CV_analysis <- LL_plate_A1_edgeless_max %>% 
  group_by(Variable) %>% 
  # select(Variable, Value) %>% 
  summarise(CV = sd(Value) / mean(Value) * 100) %>% 
  gt() %>% 
...
LL_A1_Z_data_edgeless <- LL_plate_A1_edgeless %>% 
  filter(Compound %in% c("dmso", "TAK3mM")) %>% 
  mutate(Treatment = interaction(LPS_status, Compound)) %>% 
...
# Plot (no edges)
LL_A1_Z_plot_edgeless <- LL_A1_Z_data_edgeless %>% 
  ggplot(aes(x=Treatment, y=Value, fill = Treatment)) +
  geom_boxplot() +
  geom_jitter(position = position_jitter(width = 0.2), size = 1, alpha = 0.5) +
  facet_grid(~ Threshold, labeller = labeller(Threshold = c("cell_area_1k_rod_h_mglia_dapi_cy5_23" = "1k Threshold", "cell_area_2k_rod_h_mglia_dapi_cy5_26" = "2k Threshold", "cell_area_3_5k_rod_h_mglia_dapi_cy5_30" = "3.5k Threshold"))) +
  # theme_minimal() +
  theme(legend.position = "none", strip.text = element_text(size = 12, face = "bold"), axis.title=element_text(size=13)) +
  labs(title ="Plate LL A1 (without edges)", y = "CD38 Area/nuclei") 
